...
int main(int argc, char* argv[]){ 
  ...
  ksamp();
   ...
   while ((option = getopt(argc, argv,"l:s")) != -1) { //tengo que poner los cases y si llevan mas argumentos : (por cada uno?)
        switch (option) {           
             case 'l' : interval = atoi(optarg);  //me trae el primer parametro
                  printf("%s %d \n","ksamp pro con parametros",interval);
                 break;
             case 's' : printf("ksamp con agregado \n");
                 break;
             
             default: printf("comando erroneo \n");
        }
        }
...
printf(search("vendor_id"));
rewind(Fd);
printf(search("model name"));
fclose(Fd);

openFile("/proc/version");
printf(search("version"));
fclose(Fd);
...

   {
            ...
            for(int i=0;i<strlen(text);i++){
...

               {
                  ...
                  while ((searchedWord[tmp1]==text[tmp2])&&(tmp2<strlen(text))&&(tmp1!=strlen(searchedWord))) 
